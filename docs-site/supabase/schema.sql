...
create table if not exists public.docs_sentences (
  user_id uuid not null references auth.users(id) on delete cascade,
  id uuid not null,
...
create index if not exists docs_sentences_user_idx
  on public.docs_sentences (user_id, pattern)
  where deleted_at is null;
...
create or replace function public.docs_sentences_keep_newest()
returns trigger
language plpgsql
set search_path = public
as $$
...
drop trigger if exists docs_sentences_keep_newest on public.docs_sentences;
create trigger docs_sentences_keep_newest
  before update on public.docs_sentences
  for each row execute function public.docs_sentences_keep_newest();
...
alter table public.docs_sentences enable row level security;
...
drop policy if exists "Users can read their own docs sentences" on public.docs_sentences;
drop policy if exists "Users can insert their own docs sentences" on public.docs_sentences;
drop policy if exists "Users can update their own docs sentences" on public.docs_sentences;

create policy "Users can read their own docs sentences"
  on public.docs_sentences for select
  using (auth.uid() = user_id);

create policy "Users can insert their own docs sentences"
  on public.docs_sentences for insert
  with check (auth.uid() = user_id);

create policy "Users can update their own docs sentences"
  on public.docs_sentences for update
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);
