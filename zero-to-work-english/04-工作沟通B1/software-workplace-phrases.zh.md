...
这是按需查阅的手册，不是从头背到尾的清单。下面是练习顺序，不对应 CEFR 等级；需要引导时，先做[字幕起步练习](software-meeting-demo-14-day.zh.md)。

| 练习层级 | 先学哪里 | 用来做什么 |
|---|---|---|
| 1：短回复 | 下表，再选第 1 节的屏幕、声音问题和第 5 节的澄清表达。 | 看懂短字幕、报告状态、核对意思。 |
| 2：日常协作 | 第 6 节需求与范围、第 9 节站会；只选当前任务需要的表达。 | 说明阻塞、时间、范围和下一步。 |
| 3：扩展讨论 | 第 3 节技术解释、第 4 节 Demo，以及较复杂的问答。 | 按需解释原因、备选方案和技术取舍。 |

### 第 1 层：先理解，再按事实回复

| 字幕或问题 | 参考回复 | 适用情况 |
|---|---|---|
| Can you check the logs? 你能检查日志吗？ | Yes, I can do that. 好，我可以。 | 确实能接这个任务。 |
| Is the fix ready? 修复好了吗？ | Not yet. I'm still working on it. 还没有，我还在处理。 | 仍在进行中。 |
| Can we release today? 我们今天可以发布吗？ | I need to check. 我需要查一下。 | 尚不能确认是否准备好。 |
| Could you update the ticket? 你能更新工单吗？ | Which ticket do you mean? 你指的是哪个工单？ | 不清楚具体指哪个。 |
| Can you finish by Thursday? 你能在周四前完成吗？ | Do you mean this Thursday? 你指的是本周四吗？ | 日期需要确认；`by Thursday` 表示不晚于周四。 |
| 字幕不完整或不清楚。 | Could you repeat the last part? 可以再说一遍最后那部分吗？ | 信息遗漏，不假装听懂。 |

每天选两三条问题，先解释意思，再按给定情况回复。然后改变一个事实，换一个回复。先从一句话和可选提示卡开始，60 秒汇报属于后续扩展，不是入门门槛。看英文字幕听同句音频，重要日期或指令必要时请对方写入聊天。只使用虚构或获准使用的材料。
...
1. 每天选择一个工作场景。
2. 先读懂提问，再按事实选择回复；看英文听音频并练习短回复。
3. 用自己工作中的系统、日期和任务替换例句中的内容。
4. 先录一句准确回复，可看提示卡；能力允许时再扩展为 60 秒汇报。
5. 当周至少在真实会议或聊天中使用一句。
